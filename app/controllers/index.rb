...
get '/google_login' do
  google_url = "https://accounts.google.com/o/oauth2/auth?" +
                    "response_type=code&"+
                    "client_id=#{CLIENT_ID}&"+
                    "redirect_uri=http://dbc-whack.herokuapp.com/logged_in&"+
...
get '/logged_in' do
  ...
  first_response = HTTParty.post("https://accounts.google.com/o/oauth2/token?", {body: {client_id: CLIENT_ID, client_secret: CLIENT_SECRET, redirect_uri: "http://dbc-whack.herokuapp.com/logged_in", grant_type:"authorization_code", code: access_code}})
...
get '/highscores' do
  @users = User.all
  @users = @users.sort_by{|user| user.high_score}
  @users.reverse!
  @users = @users.take(10)
